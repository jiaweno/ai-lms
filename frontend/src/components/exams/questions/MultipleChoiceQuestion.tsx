...
interface MultipleChoiceQuestionProps {
  ...
  answer?: string[] // Stores an array of selected option IDs
...
export const MultipleChoiceQuestion: React.FC<MultipleChoiceQuestionProps> = ({
  ...
  const handleOptionChange = (optionId: string, checked: boolean) => {
    ...
    if (checked) {
      newAnswer = answer.includes(optionId) ? answer : [...answer, optionId]
    } else {
      newAnswer = answer.filter(id => id !== optionId)
    }
  ...
  const getOptionClassName = (optionId: string, isCorrectOption?: boolean) => {
    ...
    const isSelected = answer.includes(optionId)
        ...
        >
          <div className="flex items-start gap-3">
            <input
              ...
              onChange={(e) => !disabled && handleOptionChange(option.id, e.target.checked)}
              disabled={disabled}
